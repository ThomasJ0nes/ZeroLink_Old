    ...
    <div className="flex flex-col min-h-screen">
      <main className="flex-1">
        <section className="bg-gray-100 py-12 px-4 text-center">
          <h1 className="text-4xl font-bold mb-4">Pay for Subscriptions on any Chain from any Chain</h1>
          <p className="text-xl mb-8">
            Pay for subscriptions on you prefered chain regardless if the subscriptions exists on a different chain.
          </p>
                ...
                <CardContent>
                  ...
                  <div className="flex space-x-2 mt-2">
                    {subscription.chains.map((chain: string) => (
                      ...
                      >
                        {chain === "Ethereum" && <Ethereum className="w-4 h-4 mr-1" />}
                        {chain === "Polygon" && <Polygon className="w-4 h-4 mr-1" />}
                        {chain}
                      </span>
                    ))}
                  </div>
                ...
                <CardFooter>
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button className="w-full" onClick={() => setSelectedSubscription(subscription)}>
                        Subscribe
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="bg-white">
                      <DialogHeader>
                        ...
                        <DialogDescription>{selectedSubscription?.description}</DialogDescription>
                      </DialogHeader>
                      <div className="mt-4">
                        <p className="font-bold mb-2">
                          {selectedSubscription?.price}/{selectedSubscription?.frequency}
                        </p>
                        ...
                        <div className="flex space-x-2">
                          {selectedSubscription?.chains.map((chain: string) => (
                            ...
                            >
                              {chain === "Ethereum" && <Ethereum className="w-4 h-4 mr-1" />}
                              {chain === "Polygon" && <Polygon className="w-4 h-4 mr-1" />}
                              {chain === "Binance" && <BNB className="w-4 h-4 mr-1" />}
                              {chain}
                            </span>
                          ))}
                        </div>
                      </div>
                      <Button className="mt-4 w-full">Confirm Subscription</Button>
                    </DialogContent>
                  </Dialog>
                </CardFooter>
              </Card>
