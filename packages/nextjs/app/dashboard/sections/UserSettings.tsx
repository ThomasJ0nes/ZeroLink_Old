...
export default function UserSettings() {
  ...
  const addWallet = () => {
    if (newWalletAddress) {
      setWallets([...wallets, { id: Date.now(), address: newWalletAddress, isPrimary: false }]);
      setNewWalletAddress("");
    }
  };
                  ...
                  {wallets.map(wallet => (
                    <div key={wallet.id} className="flex items-center justify-between">
                      <div>
                        ...
                        {wallet.isPrimary && <span className="text-sm text-green-500">Primary</span>}
                      </div>
                      <div>
                        {!wallet.isPrimary && (
                          <Button variant="outline" className="mr-2" onClick={() => setPrimaryWallet(wallet.id)}>
                            Set as Primary
                          </Button>
                        )}
